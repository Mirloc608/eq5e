const MOD = "eq5e-class-beastlord";
...
export async function generateBeastlordPacks() {
  const features = await _fetchJSON(_modulePath("data/beastlord-features.json"));
  const spells = await _fetchJSON(_modulePath("data/beastlord-spells.json"));
...
export async function mergeBeastlordAAsIntoSharedPack() {
  const aas = await _fetchJSON(_modulePath("data/beastlord-aas.json"));
