...
// tools/enforce-import-order.js
// Ensures all import statements in .js files are sorted alphabetically.

export default async function enforceImportOrder({ strict = false } = {}) {
  ...
  try {
    ...
    function processFile(filePath) {
      ...
      // Rebuild file: sorted imports + rest of file
    ...
    if (changedFiles.length > 0) {
      console.log("✔ enforce-import-order fixed:");
      for (const f of changedFiles) {
        console.log(" - " + path.relative(root, f));
      }
    } else {
      console.log("✔ enforce-import-order passed");
    }
  } catch (err) {
    console.log("⚠ enforce-import-order encountered an error");
    if (strict) throw err;
  }
}
