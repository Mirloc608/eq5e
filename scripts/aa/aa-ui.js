...
const meets = (doc) => {
  ...
  const req = aa.prereq ?? {};
  // Racial prereq scaffold (classic): req.raceId or req.races[]
  const actorRace = String(actor.flags?.eq5e?.raceId ?? "").toLowerCase();
  if (req.raceId && actorRace !== String(req.raceId).toLowerCase()) return false;
  if (Array.isArray(req.races) && req.races.length) {
    const ok = req.races.map(r=>String(r).toLowerCase()).includes(actorRace);
    if (!ok) return false;
  }
