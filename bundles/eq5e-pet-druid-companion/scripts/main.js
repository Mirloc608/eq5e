const MODULE_ID = "eq5e-pet-druid-companion";

const BASE = "systems/eq5e/bundles/eq5e-pet-druid-companion";
...
export async function generateDruidCompanionPacks() {
  const pets = await _fetchJSON(_modulePath(MOD, "data/druid-companions.json"));
  const abilities = await _fetchJSON(_modulePath(MOD, "data/druid-companion-abilities.json"));
